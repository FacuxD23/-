...
function Disable-Svc {
    ...
    if ($svc) {
        Stop-Service  -Name $Name -Force -EA SilentlyContinue | Out-Null
        Set-Service   -Name $Name -StartupType Disabled -EA SilentlyContinue
        Write-OFF "$Desc"
        if ($Riesgo) { $global:Riesgos += "  [!] $Desc ($Name): $Riesgo" }
    } else { Write-SKIP "No encontrado: $Name" }
}
function Set-Manual {
    param([string]$Name, [string]$Desc)
    $svc = Get-Service -Name $Name -EA SilentlyContinue
    if ($svc) { Set-Service -Name $Name -StartupType Manual -EA SilentlyContinue; Write-MAN "$Desc" }
    else       { Write-SKIP "No encontrado: $Name" }
}
function Disable-SvcReg {
    param([string]$Name, [string]$Desc)
    $p = "HKLM:\SYSTEM\CurrentControlSet\Services\$Name"
    if (Test-Path $p) { Set-ItemProperty -Path $p -Name "Start" -Value 4 -EA SilentlyContinue; Write-OFF "$Desc (via registro)" }
    else              { Write-SKIP "Registro no encontrado: $Name" }
}
function Set-Reg {
...
function Write-OK   { param([string]$M); Write-Host "  [OK]  $M" -ForegroundColor Green;    $global:Aplicados++ }
function Write-OFF  { param([string]$M); Write-Host "  [OFF] $M" -ForegroundColor Green;    $global:Aplicados++ }
function Write-MAN  { param([string]$M); Write-Host "  [MAN] $M" -ForegroundColor Yellow;   $global:Aplicados++ }
function Write-SKIP { param([string]$M); Write-Host "  [---] $M" -ForegroundColor DarkGray; $global:Omitidos++  }
function Write-WARN { param([string]$M); Write-Host "  [!]   $M" -ForegroundColor Yellow  }
function Write-ERR  { param([string]$M); Write-Host "  [ERR] $M" -ForegroundColor Red     }
function Write-INFO { param([string]$M); Write-Host "  [i]   $M" -ForegroundColor Cyan    }
function Write-Sub  { param([string]$T); Write-Host ""; Write-Host "  --- $T ---" -ForegroundColor DarkCyan }
