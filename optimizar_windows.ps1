...
if (Ask-YN "Instalar Microsoft Store offline?" "s") {
    ...
    $zipPath     = "$env:TEMP\msstore.zip"
    
    Write-INFO "Descargando instalador de Microsoft Store (ZIP)..."
    try {
        ...
        if (Test-Path $zipPath) {
            ...
            # El ZIP se extrae como microsoft-store-download-main/
            $extractedFolder = "$env:TEMP\microsoft-store-download-main\LTSC-Add-MicrosoftStore-master"
            
            if (Test-Path "$extractedFolder\Add-Store.cmd") {
                Write-OK "Archivos extraidos correctamente"
                $global:TempParaLimpiar += $zipPath
                $global:TempParaLimpiar += "$env:TEMP\microsoft-store-download-main"
                ...
                Write-ERR "No se encontro Add-Store.cmd en el ZIP"
            }
        } else {
            Write-ERR "No se pudo descargar el ZIP"
        }
    } catch {
...
if (Ask-YN "Abrir WinUtil en ventana separada?" "s") {
    Write-INFO "Abriendo WinUtil en nueva ventana de PowerShell..."
