...
if (Ask-YN "Aplicar optimizacion completa de red?" "s") {
    ...
    Get-NetAdapter | Where-Object { $_.Status -eq "Up" } | ForEach-Object {
        $adapterName = $_.Name
        # Desactivar power management via CIM
        $adapter = Get-CimInstance -ClassName MSPower_DeviceEnable -Namespace root/wmi -EA SilentlyContinue |
                   Where-Object { $_.InstanceName -like "*$adapterName*" }
        if ($adapter) {
            $adapter | Set-CimInstance -Property @{Enable = $false} -EA SilentlyContinue
        }
        # También deshabilitar via registro
