...
function Write-INFO { param([string]$M); Write-Host " [i]   $M"; }
function Write-ERR  { param([string]$M); Write-Host " [ERR] $M"; }
function Write-SKIP { param([string]$M); Write-Host " [---] $M"; }
...
# ================================================================
# Deteccion interna (se usa para logica HDD/SSD, Win11, etc.)
$sys = Get-SystemInfo
...
#  A. SERVICIOS BASE
# ================================================================
Write-Header "A" "SERVICIOS"
Write-Sub "Telemetria y diagnosticos"
...
if (Ask-YN "Descargar y abrir Optimizer de Hellzerg?" "s") {
    try {
...
if (Ask-YN "Instalar Microsoft Store offline?" "s") {
    $storeFolder = "$env:TEMP\MSStoreInstaller"
    $storeRepo   = "microsoft-store-download"
    $storeFiles  = @(
        "Add-Store.cmd",
        ...
        "Microsoft.NET.NativeFramework.1.6_1.6.24903.0_x64__8wekyb3d8bbwe.Appx",
        "Microsoft.NET.NativeFramework.1.6_1.6.24903.0_x86__8wekyb3d8bbwe.Appx",
        "Microsoft.NET.NativeRuntime.1.6_1.6.24903.0_x64__8wekyb3d8bbwe.Appx",
        "Microsoft.NET.NativeRuntime.1.6_1.6.24903.0_x86__8wekyb3d8bbwe.Appx",
    ...
    Write-INFO "Creando carpeta temporal en: $storeFolder"
    New-Item -Path $storeFolder -ItemType Directory -Force | Out-Null
    $global:TempParaLimpiar += $storeFolder
    
    Write-INFO "Descargando $($storeFiles.Count) archivos de Microsoft Store..."
    $downloaded = 0
    foreach ($file in $storeFiles) {
        $url = "https://raw.githubusercontent.com/FacuxD23/$storeRepo/main/LTSC-Add-MicrosoftStore-master/$file"
        $dest = "$storeFolder\$file"
        try {
            Invoke-WebRequest -Uri $url -OutFile $dest -UseBasicParsing -EA Stop
            $downloaded++
            Write-Host "  [$downloaded/$($storeFiles.Count)] $file" -ForegroundColor Gray
        } catch {
            Write-WARN "Fallo descarga: $file"
        }
    }
    
    if (Test-Path "$storeFolder\Add-Store.cmd") {
        Write-OK "Microsoft Store: $downloaded/$($storeFiles.Count) archivos descargados"
        Write-INFO "Ejecutando instalador de Microsoft Store..."
        Start-Process -FilePath "$storeFolder\Add-Store.cmd" -WorkingDirectory $storeFolder -Wait
        Write-OK "Microsoft Store instalada"
    } else {
        Write-ERR "No se pudo descargar Add-Store.cmd"
    }
} else {
...
if (Ask-YN "Instalar Rytunex?" "s") {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-OK "Rytunex instalado correctamente"
            
            if (Ask-YN "Abrir Rytunex ahora?" "s") {
                Start-Sleep -Seconds 2
                $rytunexPaths = @(
                ...
                Write-SKIP "Apertura de Rytunex omitida"
            }
        } else {
            Write-ERR "Error instalando Rytunex"
        }
    } catch {
        Write-ERR "Fallo Rytunex: $($_.Exception.Message)"
    }
} else {
...
if (Ask-YN "Instalar Winhancer?" "s") {
    $winhancerUrl  = "https://raw.githubusercontent.com/FacuxD23/-/main/Winhance.Installer.exe"
    $winhancerPath = "$env:TEMP\Winhance.Installer.exe"
    
    Write-INFO "Descargando Winhance.Installer.exe..."
    try {
        Invoke-WebRequest -Uri $winhancerUrl -OutFile $winhancerPath -UseBasicParsing
        if (Test-Path $winhancerPath) {
            $mb = [math]::Round((Get-Item $winhancerPath).Length / 1MB, 2)
            Write-OK "Winhancer descargado ($mb MB)"
            $global:TempParaLimpiar += $winhancerPath
            
            if (Ask-YN "Abrir instalador de Winhancer ahora?" "s") {
                Write-INFO "Ejecutando instalador de Winhancer..."
                Start-Process -FilePath $winhancerPath
                Write-OK "Winhancer lanzado"
            } else {
                Write-SKIP "Apertura de Winhancer omitida"
            }
        } else {
            Write-ERR "No se guardo Winhancer"
        }
    } catch {
        Write-ERR "Error descargando Winhancer: $($_.Exception.Message)"
    }
} else {
...
if (Ask-YN "Instalar WinTweaks?" "s") {
    $wintweaksUrl  = "https://raw.githubusercontent.com/FacuxD23/-/main/Setup_WinTweaks_2.1.exe"
    $wintweaksPath = "$env:TEMP\Setup_WinTweaks_2.1.exe"
    
    Write-INFO "Descargando Setup_WinTweaks_2.1.exe..."
    try {
        Invoke-WebRequest -Uri $wintweaksUrl -OutFile $wintweaksPath -UseBasicParsing
        if (Test-Path $wintweaksPath) {
            $mb = [math]::Round((Get-Item $wintweaksPath).Length / 1MB, 2)
            Write-OK "WinTweaks descargado ($mb MB)"
            $global:TempParaLimpiar += $wintweaksPath
            
            if (Ask-YN "Abrir instalador de WinTweaks ahora?" "s") {
                Write-INFO "Ejecutando instalador de WinTweaks..."
                Start-Process -FilePath $wintweaksPath
                Write-OK "WinTweaks lanzado"
            } else {
                Write-SKIP "Apertura de WinTweaks omitida"
            }
        } else {
            Write-ERR "No se guardo WinTweaks"
        }
    } catch {
        Write-ERR "Error descargando WinTweaks: $($_.Exception.Message)"
    }
} else {
...
if (Ask-YN "Instalar Wintoys desde Microsoft Store?" "s") {
    Write-INFO "Instalando Wintoys via winget..."
    try {
        winget install --id 9P8LTPGCBZXD --accept-package-agreements --accept-source-agreements --silent
        if ($LASTEXITCODE -eq 0) {
            Write-OK "Wintoys instalado correctamente"
            
            if (Ask-YN "Abrir Wintoys ahora?" "s") {
                Start-Sleep -Seconds 2
                ...
                Write-SKIP "Apertura de Wintoys omitida"
            }
        } else {
            Write-ERR "Error instalando Wintoys"
        }
    } catch {
        Write-ERR "Fallo Wintoys: $($_.Exception.Message)"
    }
} else {
